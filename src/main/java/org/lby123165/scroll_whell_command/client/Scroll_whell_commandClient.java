...
public class Scroll_whell_commandClient implements ClientModInitializer {
    ...
    // Track previous state to edge-detect the open key, avoiding key-repeat flicker
    ...
    private void onClientTick(MinecraftClient client) {
        ...
        // Edge-detect the open key to support both mouse and keyboard without repeat toggling
        ...
        if (RadialMenuOverlay.isVisible()) {
            ...
            // The menu is not open, open on press edge only (prevents repeats)
            if (openKeyDown && !openKeyDownPrev) {
